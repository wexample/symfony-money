...
**Editing.** `CurrencyForm` binds five fields to `data_class => Currency::class` under the translation domain `WexampleSymfonyMoneyBundle.forms.currency_form`, with the `type` select fed by `Currency::getAllowedTypes()`. The row it edits is loaded by `EntityFormDataResolver` from `wexample/symfony-forms`, which `#[EntityFormProcessor(CurrencyFormProcessor::class, Currency::class)]` wires on the controller method — the bundle carries no resolver of its own. On submit, `CurrencyFormProcessor::onValid()` re-checks code uniqueness against the repository and, on collision, calls `addFormErrorFromApiKey($form, 'ERR_CURRENCY_CODE_ALREADY_USED')` — the key resolved by `assets/forms/currency_form.en.yml`. Otherwise it persists, flushes, and sets the success action and notification.
